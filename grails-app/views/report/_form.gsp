...
<div class="row">
	<div class="form-group col-md-6 ${hasErrors(bean: reportInstance, field: 'queryType', 'error')} required">
		...
		<g:select class="form-control" name="queryType" from="${br.com.acception.greport.QueryType?.values()}" keys="${br.com.acception.greport.QueryType.values()*.name()}" required="" value="${reportInstance?.queryType?.name()}"/>
	</div>
