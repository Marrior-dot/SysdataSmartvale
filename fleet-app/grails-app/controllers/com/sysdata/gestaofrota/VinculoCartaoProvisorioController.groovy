...
class VinculoCartaoProvisorioController {
    ...
    def unlinkFromCardHolder() {
        ...
        try {
            ...
            log.info "Cartão provisório #${card.id} desvinculado ao Portador #${cardHolder.id}"
            render status: HttpStatus.SC_OK, text: "Cartão provisório #${card.id} desvinculado ao Portador #${cardHolder.id}"
